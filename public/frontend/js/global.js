...
(function ($) {
    ...
    try {
		...
		$('.menu-mobile .menu-item > a').on('click', function () {
            $('.header-nav-menu-mobile').toggleClass('is-active');
            $('.header-nav-menu-mobile').slideToggle('500');
        });		
    ...
    try {
        ...
        var MobHeaderWrapper = $(".header-mobile");

        MobHeaderWrapper.each(function () {
            var that = $(this);
            var MobHeaderbar = that.find('.header-mobile__bar');

            var MobHeaderbarOffsetTop = MobHeaderbar.offset().top;
            var MobHeaderbarHeight = MobHeaderbar.outerHeight();

            MobHeaderbar.after("<div class='header__holder'></div>");

            $(window).on('scroll', function () {
                if ($(window).scrollTop() > MobHeaderbarOffsetTop) {
                    //$('.header__holder').css('height', MobHeaderbarHeight);
                    MobHeaderbar.addClass('header--fixed');
                } else {
                    MobHeaderbar.removeClass("header--fixed");
                    //$('.header__holder').css('height', 0);
                }
            });
        });
